    ...
    try {
      if (isDeleteAction) {
        ...
        for (const id of selectedIds) {
          try {
            await parentService.deleteById(id);
            successCount++;
          } catch (error) {
        ...
        if (successCount > 0) {
          toast.success(`Successfully deleted ${successCount} parent(s)`);
        }
        if (failCount > 0) {
          toast.error(`Failed to delete ${failCount} parent(s)`);
        }
      } else {
        // Status update
        ...
        toast.success(
          `Successfully updated ${selectedIds.length} parents to ${bulkAction}`,
        );
      }
      ...
      toast.error(
        isDeleteAction
          ? "Failed to delete parents. Please try again."
          : "Failed to update parents. Please try again.",
      );
    } finally {
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          >
            ...
            {isDeleteAction ? "Delete Selected Parents" : "Bulk Status Update"}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {isDeleteAction ? (
              <>
                ...
                <strong>{selectedIds.length}</strong> parent(s). This action
                cannot be undone. All associated accounts will also be removed.
              </>
            ) : (
              <>
                Are you sure you want to set{" "}
                <strong>{selectedIds.length}</strong> parents to{" "}
