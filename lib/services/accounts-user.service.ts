...
export interface AccountsAccountInput {
  name: string;
  email: string;
  password: string;
}
...
class AccountsUserService {
  async getAll(): Promise<User[]> {
    ...
    return users.filter((user) => user.role === "accounts");
  }

  async create(data: AccountsAccountInput, actionBy = "admin"): Promise<string> {
...
export const accountsUserService = new AccountsUserService();
